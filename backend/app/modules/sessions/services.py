...
async def create_session(db: DbSession, title: str, agent_id: int):
    ...
    Args:
        ...
        agent_id (int): User ID of the agent creating the session.

    Returns:
        Session: The new session object.

    Raises:
        HTTPException: If the LiveKit room creation fails.
    """
    ...
    session = repositories.create_session(
        db, title=title, room_name=room_name, agent_id=agent_id
    )
    logger.info(
        "session_created", session_id=session.id, room=room_name, agent_id=agent_id
    )
    # Increment metric
    try:
...
def get_join_link(session) -> str:
    ...
    Returns:
        str: The relative join link URL wrapper.
    """
...
async def end_session(db: DbSession, session_id: int, agent_id: int):
    ...
    This handles broadcasting the end notification, cleaning up LiveKit resources,
    and automatically stopping any active egress recordings.

    Args:
        db (DbSession): Database session.
        session_id (int): The ID of the session to end.
        agent_id (int): The ID of the agent requesting the end.
    ...
    try:
        ...
        logger.warning(
            "livekit_delete_room_failed", error=str(e), room=session.room_name
        )
        ...
        for rec in active_recordings:
            if rec.status == "recording":  # Using string literal or enum value check
                ...
                try:
                    ...
                    logger.warning(
                        "auto_stop_recording_failed", error=str(e), recording_id=rec.id
                    )
    except Exception as e:
...
def create_session_archive(db: DbSession, session_id: int):
    """Compile session chat history and attached media into a zip archive.
    ...
    Returns:
        tuple[io.BytesIO, str]: A tuple containing the in-memory archive stream
    ...
    with zipfile.ZipFile(memory_file, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        chat_lines = []
        chat_lines.append(
            f"Chat History for Session: {session.title} (ID: {session.id})"
        )
        ...
        for msg in messages:
            ...
            if msg.message_type == "file":
                chat_lines.append(
                    f"[{timestamp}] {sender_name}: [Shared File: {msg.content}]"
                )
            else:
